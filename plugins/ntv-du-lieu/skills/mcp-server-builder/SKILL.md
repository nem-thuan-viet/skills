...
description: "Design and ship production-ready MCP (Model Context Protocol) servers from OpenAPI contracts instead of hand-written tool wrappers. Python and TypeScript support, schema validation, safe evolution. Use when exposing an existing API as an MCP server, building tool integrations for Claude or Codex or Cursor, or scaffolding an MCP project from scratch. Hãy dùng khi anh Bình nói 'dựng cổng MCP', 'tạo MCP server', 'làm cổng cho AI', 'biến API thành tool cho Claude', 'thêm tool vào cổng FAC'."
---

## 🏭 Bối cảnh NTV — đọc & ÁP trước khi chạy skill này

> **Nhãn tiếng Việt:** Dựng Server MCP
> Skill này chạy trong dự án **Thuần Việt Platform** (Nệm Thuần Việt). Các sự thật sau áp trước khi làm:

- **Đã có sẵn cổng MCP `fac-ntv`** nối thẳng ERPNext (Frappe v16 chạy **docker**, site `ntv`). MCP mới nên **theo mẫu đó**.
- **An toàn token:** khóa/token lấy từ kho DB `core.platform_credentials` hoặc `~/.ntv-secrets/` — **CẤM hardcode, CẤM commit lên git**.
- **Bài học phân quyền:** phải cấp **Role nghiệp vụ** đúng (vai `System Manager` KHÔNG đọc được `Customer`/`Item`).
- **Lên VM sản xuất** phải siết tài khoản + qua **`/nghiem-thu`** trước; agent không tự đụng VM.
- **Luật đặt tên:** tên kỹ thuật = tiếng Anh; nhãn hiển thị = tiếng Việt có dấu.
