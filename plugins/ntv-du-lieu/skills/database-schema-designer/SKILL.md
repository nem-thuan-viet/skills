...
description: "Use when the user asks to create ERD diagrams, normalize database schemas, design table relationships, or plan schema migrations. Hãy dùng khi anh Bình nói 'vẽ ERD', 'sơ đồ quan hệ', 'thiết kế bảng', 'thiết kế database', 'chuẩn hóa', 'quan hệ giữa các bảng', 'kế hoạch migrate schema'."
---

## 🏭 Bối cảnh NTV — đọc & ÁP trước khi chạy skill này

> **Nhãn tiếng Việt:** Thiết Kế Lược Đồ CSDL
> Skill này chạy trong dự án **Thuần Việt Platform** (Nệm Thuần Việt). Các sự thật sau **THẮNG** mọi mặc định chung của skill — áp trước khi làm:

- **CSDL đích = PostgreSQL THUẦN**, viết SQL/DDL tay. Repo **KHÔNG dùng ORM** (Prisma/Drizzle/TypeORM/SQLAlchemy) → **bỏ qua toàn bộ phần ORM** trong skill.
- **Kho phân tích 4 tầng**: `raw → dim → fact → mart`; DDL đặt ở thư mục `sql/` chạy theo số `00→99`; các bảng nối nhau bằng khóa **`team_key`**.
- ⚠️ **BẪY MÚI GIỜ (bắt buộc nhớ):** DB lưu **UTC**. Mọi phép cắt ngày `::date` trên cột thời gian PHẢI kèm `AT TIME ZONE 'Asia/Ho_Chi_Minh'` — từng gây lệch doanh thu POS 35,3%.
- **Chạy thử SQL:** qua proxy Cloud SQL cổng `5433` (Cockpit đã ngừng dùng).
- **CÓ CSDL THỨ 2** trong dự án = **MariaDB của ERPNext/Frappe** (dữ liệu DocType) — KHÁC kho Postgres, đừng lẫn.
- **Luật đặt tên:** tên kỹ thuật (bảng, cột) = tiếng Anh; nhãn hiển thị người dùng = tiếng Việt có dấu.
- **An toàn:** CẤM hardcode token vào SQL/script; token nằm ở `core.platform_credentials`.

*Nguồn skill: alirezarezvani/claude-skills (MIT), kéo 15/07/2026. Ruột gốc giữ nguyên bên dưới.*
